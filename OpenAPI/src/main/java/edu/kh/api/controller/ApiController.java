...
public class ApiController {
	...
	public String ex2(
		...
		Model model // :: spring에서 값을 전달
		) throws IOException, JSONException {
		
		String serviceKey= "WCQYQB3bkrkzZfjc6UGwBUSEyH6oDSWPkqxO36vcZJN3K0nA6PTt90Sc9%2FKEX0eAbwa94oMljoUwJ2Vn4Bv4dA%3D%3D";
		int numOfRows = 1000; // 조회할 데이터 개수
		int pageNo = 1; // 조회할 페이지(기상청은 페이지 1장)
		String dataType = "JSON"; // 응답 데이터 타입 지정(JSON/XML 두가지만 가능)
		...
		String[] arr = (sdf.format(new Date())).split("/"); // java.util.date
		...
		// StringBuilder : String의 단점(불변성)을 해결한 객체 :: string으로 붙이면 메모리 문제
        ...
        // 입력 보조 스트림
        BufferedReader rd; // 입력 
        
        // 응답 상태 코드가 200~300번대인 경우 (요청/응답 성공인 경우)
        ...
        // 데이터를 한 줄 씩 계속 읽어 들여서 sb(StringBuilder)에 추가
        // 더 이상 읽을 내용이 없으면 반복문을 종료(하라는 구문)
        ...
        //System.out.print(j1.getString("response"));  // string 타입이 된다
        ...
        String item = j4.getString("item"); // JSON(문자열) 로 얻어옴
        // :: dto를 엄청 만들어서 꺼내는 법도 있긴하다...
        ...
        weatherMap.put("fcstDate", list.get(0).get("fcstDate")); // 20240521:: 가장 가까운 시간대의 값 가져오기
        weatherMap.put("fcstTime", list.get(0).get("fcstTime")); // 0900
        
        // 조회된 많은 데이터 중
        // 현재 시간으로 부터 가장 가까운 시간대의 데이터만 모아두기
