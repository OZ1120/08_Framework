...
--회원 목록 조회
SELECT MEMBER_NO,MEMBER_EMAIL, MEMBER_NICKNAME, MEMBER_DEL_TEL;
...
CREATE TABLE "MEMBER" (
	...
	"MEMBER_NICKNAME"	NVARCHAR2(10)		NOT NULL,
	"MEMBER_TEL"	CHAR(11)		NOT NULL,
	"MEMBER_ADDRESS"	NVAARCHAR2(300)		NULL,
...
COMMENT ON COLUMN "MEMBER"."AUTHORITY" IS '권한(1:일반,2관리자)';

CREATE TABLE "UPLOAD_FILE" (
	"FILE_NO"	NUMBER		NOT NULL,
	"FILE PATH"	VARCHAR2		NOT NULL,
	"FILE_ORIGINAL_NAME"	VARCHAR2		NOT NULL,
	"FILE_RENAME"	VARCHAR2(100)		NOT NULL,
	"FILE_UPLOAR_DATE"	DATE	DEFAULT SYSDATE	NOT NULL,
	"MEMBER_NO"	NUMBER		NOT NULL
);

COMMENT ON COLUMN "UPLOAD_FILE"."FILE_NO" IS '파일번호(PK)';

COMMENT ON COLUMN "UPLOAD_FILE"."FILE PATH" IS '파일 요청 경로';

COMMENT ON COLUMN "UPLOAD_FILE"."FILE_ORIGINAL_NAME" IS '파일 원본명';

COMMENT ON COLUMN "UPLOAD_FILE"."FILE_RENAME" IS '파일 변경명';

COMMENT ON COLUMN "UPLOAD_FILE"."FILE_UPLOAR_DATE" IS '업로드 날짜';
...
SELECT BOARD_NO,BOARD_TITLE,MEMBER_NICKNAME, READ_COUNT,
		...
		CASE 
			...
			THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24 * 60 * 60) || '초전'
			
			WHEN SYSDATE - BOARD_WRITE_DATE <1 / 24 
			THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24 * 60) || '분전'
...
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 댓글 1',DEFAULT
			,DEFAULT,
			2014,3,	NULL);
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 댓글 2',
			DEFAULT,DEFAULT,2014,3,NULL);
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 댓글 3',
		...
		-- 부모댓글 1의 자식 댓글
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 1의 자식1',
				DEFAULT,DEFAULT,2014,	3,2004);
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 1의 자식2',
				DEFAULT,DEFAULT,2014,	3,2004);
			
			
-- 부모댓글 2의 자식 댓글
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 2의 자식1',
				DEFAULT,DEFAULT,2014,	3,2005);
			
-- 부모댓글 2의 자식1의 자식 댓글
INSERT INTO "COMMENT" 
VALUES ( SEQ_COMMENT_NO.NEXTVAL,'부모 2의 자식1의 자식',
				DEFAULT,DEFAULT,2014,	3,2009);
...
-- PARENT_COMMENT_NO값이 NULL인 행이 부모(LV.1)
START WITH PARENT_COMMENT_NO IS NULL 
-- 부모의 COMMENT_NO와 같은 PARENT_COMMENT_NO를 가진 행을 자식으로 지정
CONNECT BY PRIOR  COMMENT_NO = PARENT_COMMENT_NO --:: 부모자식으로 연결
...
SELECT CHATTING_NO
			,(SELECT MESSAGE_CONTENT FROM (
				...
				WHERE M2.CHATTING_NO = 1 -- 상관쿼리, 서브쿼리안에 서브쿼리
				ORDER BY MESSAGE_NO DESC) 
				WHERE ROWNUM = 1) LAST_MESSAGE -- 마지막 메세지
			,TO_CHAR(NVL((SELECT MAX(SEND_TIME) SEND_TIME -- 마지막 날짜/시간이 큰값(미래)
					FROM MESSAGE M
					WHERE R.CHATTING_NO  = M.CHATTING_NO), CREATE_DATE), 
					'YYYY.MM.DD') SEND_TIME
			,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
						WHERE R2.CHATTING_NO = 1
						AND R2.OPEN_MEMBER = 1),
						R.PARTICIPANT, -- NULL x / 참여자(메세지 받는대상)
						R.OPEN_MEMBER -- NULL o / 오픈자(메세지받는대상)
				) TARGET_NO	-- 메세지 받는 사람
			,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
				...
				) TARGET_NICKNAME	-- 타겟 닉네임
			,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
				WHERE R2.CHATTING_NO = R.CHATTING_NO
				AND R2.OPEN_MEMBER = #{memberNo}),
				(SELECT PROFILE_IMG FROM MEMBER WHERE MEMBER_NO = R.PARTICIPANT),
				(SELECT PROFILE_IMG FROM MEMBER WHERE MEMBER_NO = R.OPEN_MEMBER)
				) TARGET_PROFILE -- 타겟 프로필
			,(SELECT COUNT(*) FROM MESSAGE M 
				WHERE M.CHATTING_NO = R.CHATTING_NO 
				AND READ_FL = 'N' 
				AND SENDER_NO != #{memberNo}) NOT_READ_COUNT -- 안읽음 표시
			,(SELECT MAX(MESSAGE_NO) SEND_TIME FROM MESSAGE M 
				WHERE R.CHATTING_NO  = M.CHATTING_NO) MAX_MESSAGE_NO-- 마지막 메세지 번호
		FROM CHATTING_ROOM R
		WHERE OPEN_MEMBER = #{memberNo}
		OR PARTICIPANT = #{memberNo}
		ORDER BY MAX_MESSAGE_NO DESC NULLS LAST -- 마지막으로 보낸 메시지 번호 큰거 조회(대화방 나열)
