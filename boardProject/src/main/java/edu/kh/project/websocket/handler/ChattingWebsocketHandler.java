...
@Component
@Slf4j
@RequiredArgsConstructor // :: 매개변수 생성자 -> 의존성 주입
public class ChattingWebsocketHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// 연결된 클라이언트의 정보를 sessions에 추가(add) ::를 전달하는 코드
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		// 연결 종료된 클라이언트를 Sessions에서 제거(remove)
		sessions.remove(session);
	}
	
	
	// 클라이언트로 부터 텍스트 메시지를 전달 받은 경우
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
	    ...
	    // DB 삽입 서비스 호출
	     ...
	     // :: chattingNo, messageContent, sendMember, targetNo,시간
	     
		
		// :: sessions : 연결된 클라이언트 정보가 모두 모여있음 ->
		// 연결된 모든 클라이언트를 순차 접근
