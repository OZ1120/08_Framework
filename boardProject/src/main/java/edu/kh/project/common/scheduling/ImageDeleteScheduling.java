...
public class ImageDeleteScheduling {
	...
	@Value("${my.profile.folder-path}")
	private String boardLocation; // 프로필 이미지 저장 경로
	
//	@Scheduled(cron="0/10 * * * * *") // 0초 기준, 10초 마다(테스트)
	@Scheduled(cron="0 0 * * * *") // 정시 마다(0분 0초마다)
	public void imageDelete() {
		...
		File[] boardeArr = boardFolder.listFiles();
		
		// 3. 배열 -> list로 볌어ㅕㅣ
		List<File> profileList = Arrays.asList(profileArr);
		List<File> boardList   = Arrays.asList(boardeArr);
