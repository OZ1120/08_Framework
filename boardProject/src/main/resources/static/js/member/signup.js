...
memberPw.addEventListener("input", e=>{
  ...
  if(inputPw.trim().length === 0){
    ...
    memberPw.value = ""; // 처음 띄어쓰기 입력 못하레 하기
    return;
  }
...
memberNickname.addEventListener("input", e => {
  ...
  // 1) 입력 안한 경우
  if(inputNickname.trim().length ===0){// 입력 값 없을 때
  ...
  if ( !regExp.test(inputNickname)){ // 유효하지 않을때
...
// 전달 받은 숫자가 10미만 인 경우(한자리) 앞에 0붙여서 반환
  ...
  for (let key in checkObj){ // checkObj 요소의 key값을 순서대로 꺼내옴
    ...
    if( !checkObj[key]){ // false인 경우(유효하지 않음)
      ...
      switch(key){
        ...
        case "memberPwCorfirm" : str = "비밀번호가 일치하지 않습니다"; break;
...
// -> 입력된 인증번호와 발급된 인증번호 일치 여부 확인(같은지 비교)
//    같으면 1, 아니면 0 반환
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  fetch("/email/checkAuthKey", {
    method : "post",
    headers : {"Content-Type" : "application/json"},
    body : JSON.stringify(obj) // obj를 JSON 변경 \js를 java에서 못읽으므로 JSON으로 변경
    //전달할 데이터
  })
  .then(resp => resp.text())
  .then(result => {
    ...
    if(result == 0){
      ...
      checkObj.authKey = false; //\안전장치로 false넣어 두기
      return;
    }
