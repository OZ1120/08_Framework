...
if(notificationLoginCheck){ // 로그인 상태일 경우만 알림 WebSocket 수행
    ...
    sendNotificationFn = (type, url, pkNo) => {
        ...
        // :: const title = document.querySelector(".board-title").innerText; -> 변경함

        const notification = {
            "notificationType" : type, // boardLike
            "notificationUrl": url, // 주소
            "pkNo" : pkNo // 번호
        }

        notificationSock.send(JSON.stringify(notification)); //자바객체 json으로 만듬
    }
    

    /* 웹소켓을 통해 서버에서 전달된 메시지가 있을 경우 */
    notificationSock.addEventListener("message", e => {
        // const obj = JSON.parse(e.data); 쓸일 없음
        // console.log(obj)
    ...
    notReadCheckFn = async () => {
        const resp = await fetch("/notification/notReadCheck") // fetcj시 프로미스 형태
        ...
        .then(selectList => {
            ...
            for(let data of selectList){
                ...
                notiText.addEventListener("click", e => {
                    ...
                    // 만약 읽지 않은 알람인 경우
                    if(data.notificationCheck == 'N') { //::읽음으로 바꾸는 코드
                        fetch("/notification", {
                            method: "PUT",
...
/* 페이지 DOM 요소 내용이 모두 로딩된 후(화면 렌더링이 끝난 후) */
document.addEventListener("DOMContentLoaded", () => { // :: 문서 전체에 요소내용 불러왔을때(html해석 다되면)
    ...
    if (targetElement) {
        const scrollPosition = targetElement.offsetTop;//::스크롤이 몇픽셀 내려왔는지(위부터 현재요소까지의 거리)
        window.scrollTo({
            top: scrollPosition - 200, // 스크롤 화면맨 위에서부터 200px에 자리하기
            behavior: 'smooth'// 그자리까지 화면이 천천히 욺직인다
        });
    }
})
