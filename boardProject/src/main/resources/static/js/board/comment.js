...
- 자원(데이터, 파일)을 이름(주소)으로 
  ...
  자원의 상태(State)를 주고 받는 것(전송 : Transfer)
...
const selectCommentList = ()=>{
  ...
  fetch("/comment?boardNo=" + boardNo) // GET 방식 요청:: boardNo : 전역변수 boardDetail
...
const addContent = document.querySelector("#addComment")// butoon
...
addContent.addEventListener("click", e=> {
  ...
  // ajax를 이용해 댓글 등록 요청 :: 비동기 요청
  const data = {// 제출할 데이터
    "commentContent" : commentContent.value,
    "boardNo" : boardNo, // ::boardDetail 전역 변수 선언
...
const insertChildComment = (parentCommentNo, btn) =>{
  ...
  // 유효성 검사
  if(textarea.value.trim().length == 0){ // 대댓글 내용 없을때
    alert("내용 작성후 등록 버튼 클릭!");
    textarea.focus();
    return;
  }

  // 자식 댓글(답글)
  // ::댓글번호, 게시글 번호, 회원 번호, 부모댓글 번호
  // :: 변수 저장
// ajax를 이용해 댓글 등록 요청 :: 비동기 요청
const data = {// 제출할 데이터
...
const deleteComment = commentNo => {
  ...
  fetch("/comment",{
    method : "DELETE",
    headers : {"Content-Type" : "application/json"},
    body : commentNo //:: 값 하나라 json으로 변환 안시키고 보냄
  })
  .then(response => response.text()) //:: 숫자 반환
  .then(result => {

    if(result >0){
      alert("삭제 되었습니다!");
      selectCommentList(); // 다시 조회후 화면 다시 만들기
    }else{
      alert("삭제 실패");
    }
...
// 수정 취소 시 원래댓글 형태로 돌아가기 위한 백업 변수
...
const showUpdateComment = (commentNo, btn) =>{
  ...
  if(temp != null){
    ...
    if(confirm("수정 중인 댓글이 있습니다. 현재 댓글을 수정하시겠습니까?")){
      ...
      commentRow.after(beforeCommentRow); // 기존 댓글 다음에 백업추가
  ...
  const commentRow = btn.closest("li"); // 부모중 가장 가까운 li 태그찾기
  // console.log(coomentRow);
  ...
  const updateBtn = document.createElement("button");
  updateBtn.innerText = "수정";
  updateBtn.setAttribute("onclick", `updateComment(${commentNo},this)` ) // 속성추가
...
const updateCancle = (btn) => {
  ...
  if(confirm("취소 하시겠습니까?")){
    ...
    const commentRow = btn.closest("li"); // 기존 댓글 행 :: 가장 가까운 부모 li태그 행
    commentRow.after(beforeCommentRow); // 기존 댓글 다음에 백업추가
    commentRow.remove(); // 기존 삭제 -> 백업이 기존 행 위치로 이동
