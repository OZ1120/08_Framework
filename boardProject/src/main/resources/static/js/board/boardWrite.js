/* quertySelector의 문제점
  quertySelector(), querySelectorAll()은
  호출 되었을때 시점의 요소 형태를 그대로 얻어옴
...
const previewList = document.getElementsByClassName("preview"); // img 태그 5개 얻어오기 :: 미리보기

const inputImageList = document.getElementsByClassName("inputImage"); // input 태그 5개 얻어오기 :: 

const deleteImageList = document.getElementsByClassName("delete-image"); // x버튼 5개 얻어오기 :: 파일 지울것

// 이미지 선택 이후 취소를 누를 경우를 대비한 백업 이미지
// (백업 원리 -> 복제품으로 기존 요소를 대체함)
const backupInputList = new Array(inputImageList.length); // ::인풋 이미지 길이만큼 만들기
...
const changeImageFn = (inputImage, order) => {
  ...
  if(file == undefined){
    console.log("파일선택 취소됨");

    // 같은 순서(order)번째 backupInputList 요소를 얻어와 대체하기

    /* 한 번 사용된(화면에 추가된) 요소는 재사용(다른 곳에 또 추가) 불가능 */

    // 백업본을 한 번 더 복제
    const temp = backupInputList[order].cloneNode(true);

    inputImage.after(temp); // 백업본을 다음 요소로 추가 ::원본 옆 백업본
...
for(let i = 0; i<inputImageList.length; i++){
  ...
  // ***** input 태그에 이미지가 선택된 경우(값이 변경된 경우) *****
